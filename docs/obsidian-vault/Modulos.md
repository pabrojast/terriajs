...
| `lib/Models/Catalog/CatalogItems/Cog*.ts`                             | items `cog` y `cog-time-series`: estilo y leyenda, fabrica de proveedores con pools compartidos, pasos de tiempo con precarga, resoluciones, pick propio y series en punto (ver [[Flujos Importantes]], seccion 9) | `CogTimeSeriesCatalogItem.ts`, `CogProviderFactory.ts`, `CogRenderStyle.ts`        |
| `lib/Core/Cog*.ts`                                                    | piezas sin UI para COG: pools de workers compartidos, cache de GeoTIFF abiertos y estadisticas, lector de pixel y de series, motor zonal                                                                           | `CogSharedPools.ts`, `CogSourceCache.ts`, `CogPointReader.ts`, `CogZonalEngine.ts` |
| `lib/ReactViews/Custom/Chart/ChartJs/`                                | renderer Chart.js (chunk perezoso), dock inferior con KPIs y marcador de fecha, estadisticas puras                                                                                                                 | `ChartJsSeriesDock.tsx`, `ChartJsLineChart.tsx`, `chartJsStats.ts`                 |
| `lib/Models/ChartSeriesAccumulator.ts`                                | interfaz estructural de los items que alimentan el dock de graficos (CSV, series COG)                                                                                                                              | `ChartSeriesAccumulator.ts`                                                        |
